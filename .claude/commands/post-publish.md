...
- [ ] **Si el frontmatter del relato/artículo tiene `display_title:`** (regla 2026-04-26 PM, aplica a Ficciones Domésticas one-shots/miniseries y series desde el siguiente episodio): comparar el `<h1>` del post publicado contra el `display_title:` del frontmatter. **Deben coincidir.** Si Rafael pegó por error el `title:` corto en lugar del `display_title:`, el H1 web Beehiiv y el subject del email saldrán sustantivo-simples (*La objeción*) en lugar de declarativos paradójicos (*"La cuidadora que reza para que su humanoide no la sustituya antes del tribunal médico"*) — pérdida de hook compositivo. **Triaje:** si discrepancia → PARAR y avisar a Rafael con el comando para arreglarlo en Beehiiv editor (Beehiiv permite editar Title después de publicar, aunque Apple/Amazon Music no propaguen el cambio en feeds RSS — ver regla `automation.md § Descripción y título del episodio NO se actualizan al re-publish del feed`). Detalle: [`@.claude/rules/editorial.md § Display title declarativo YouTube-style`](../../.claude/rules/editorial.md).
