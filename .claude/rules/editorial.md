...
- **B5. Paridad de tratamiento visual.** Si N productos/finalistas se tratan como equiparables en el cuerpo, todos reciben **o bien** (imagen + pie + fuente) **o bien** ninguno. Asimetrías = decisión explícita documentada en `PASOS.md` (*"omitimos foto de X porque el fabricante no facilita render oficial"*), nunca olvido. Verificación pre-output: contar `figure-NN-<slug>.*` en `assets/` ↔ número de finalistas tratados.
