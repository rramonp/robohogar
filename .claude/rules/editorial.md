...
- **Display title declarativo YouTube-style — OBLIGATORIO en one-shots y miniseries futuras** (regla dura 2026-04-26 PM tras feedback Rafael viendo recomendaciones de YouTube anglo de "Domestic Fictions": *"quiero cambiar el formato de los títulos de mis próximos one-shots y miniseries… los títulos como ya hay un hook en ellos"*). Eje **independiente** del `title:` corto interno y del `slug:`. El relato lleva 4 nombres distintos en su frontmatter:
  ...
  - `display_title:` **nuevo, obligatorio** — formato declarativo paradójico, fórmula `"El/La [rol/oficio] que [acción inusual + objeto-imposibilidad]"`, 8-15 palabras. Ejemplos canon ROBOHOGAR: *"La cuidadora que reza para que su humanoide no la sustituya antes del tribunal médico"* · *"El humanoide que cobra de noche por los sueños que no tiene"* · *"La ministra que carga su humanoide en el enchufe vacío del Congreso"*.
  ...
  - **Verificación pre-output (bloqueo duro):** falta `display_title:` o `tag_poetico:` en frontmatter → output bloqueado. `display_title` < 8 o > 15 palabras → bloqueado. Subfamilia G repetida en últimos 5 sin excepción documentada → bloqueado. Display title que mencione nombre propio real de figura pública → bloqueado.
...
- **Hero de one-shots y miniseries — canon `personaje-acción-imposibilidad` para nuevos relatos** (regla dura 2026-04-26 PM, canon visual nuevo paralelo al `portada minimalista · objeto-testigo` existente). Aplica solo a one-shots y miniseries futuras. Las 3 series activas (Amparo, Ronda 3, MAIA) mantienen su código visual sin tocar.
  - **ADN visual ROBOHOGAR fijo (sigue heredado de `asset-catalog.md § 5`):** estilo fotográfico still-life cinematográfico (After Yang / Hammershoi / Wenders), paleta azul noche `#1E2A3A` matte + foco lateral cálido ámbar único, dimensiones 1200×630 (`--model 2 --aspect 16:9 --size 2K` + crop Pillow), anti-sign-guard activo (cero neones, cero caracteres asiáticos, cero LEDs/glow). **Cero adopción del estilo oil painting / digital painting anglo** del canal "Domestic Fictions" de YouTube — solo adoptamos su gramática compositiva, no su look-and-feel pintado.
