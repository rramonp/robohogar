...
- **Tangible inline con snippet HTML copy-paste + mobile-first 100 % — OBLIGATORIO** (regla dura 2026-04-23). Todo tangible inline del cuerpo (checklist, decision-tree, dossier 3-datos, cuadro sí/no, tabla standalone, formatos futuros) se entrega como **preview renderizado + `.snippet-block` con HTML en estilos inline escapado** (regla detallada en `@rules/design.md § Bloques de código`). Plantillas canónicas + checklist mobile-first 375 px + variedad editorial entre artículos consecutivos: [`content/templates/tangibles-snippets.md`](../../content/templates/tangibles-snippets.md). Skills (`/content-draft`, `/pdf-brand`) leen ese archivo al generar el tangible. Verificación pre-output (`/content-draft § 8.7`): falta snippet-block → bloqueo del output.
