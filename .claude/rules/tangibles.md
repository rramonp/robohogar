...
- **Tangible inline con snippet HTML copy-paste + mobile-first 100 % — OBLIGATORIO** (regla dura 2026-04-23). Todo tangible inline del cuerpo (checklist, decision-tree, dossier 3-datos, cuadro sí/no, tabla standalone y cualquier formato futuro) se entrega en el borrador con **dos representaciones en orden**:

  1. **Preview renderizado** usando las clases CSS del `<style>` global del borrador (ej. `.checklist`, `.decision-cards / .decision-card`). Rafael lo ve al abrir el preview en navegador y valida que encaja con el resto del artículo.
  2. **`.snippet-block` con HTML en estilos inline escapado** (`<pre><code>` con `&lt;` / `&gt;` / `&amp;`). Rafael copia este código a Beehiiv vía `/html` → "Custom HTML block". Los estilos inline son obligatorios porque las clases CSS del borrador **no viajan a Beehiiv**: si pega el preview con clases, el editor aplica su propio estilo y el tangible pierde el look ROBOHOGAR (crema + ámbar, fuente, padding).

  **Mobile-first 100 %** (80 % del tráfico ROBOHOGAR es móvil — regla `@rules/design.md § Mobile-first`). El snippet inline va sin `@media queries` (Beehiiv Custom HTML no las respeta siempre en email render): stack vertical natural, padding/font-size generosos, sin tablas multi-columna forzadas, sin ASCII art, sin elementos de ancho fijo que no quepan en 375 px. Si no renderiza legible en viewport 375 px, **se rediseña antes de entregar**. Sin excepciones.

  **Fuente de verdad de las plantillas**: [`content/templates/tangibles-snippets.md`](../../content/templates/tangibles-snippets.md). Contiene las plantillas canonizadas (checklist, decision-tree; más a medida que se usen) con preview + snippet inline listos para adaptar al contenido concreto del artículo. Skills (`/content-draft`, `/pdf-brand`, futuros) leen este archivo al generar el tangible.

  **Variedad editorial obligatoria — anti-repetición de formato entre artículos consecutivos** (ver `@.claude/commands/content-draft.md § 3 bis`). No repetir el mismo tipo de tangible en 2 de los últimos 3 artículos. Menú operativo vigente: checklist · decision-tree · dossier 3-datos · cuadro sí/no · tabla standalone. Rafael introduce nuevos formatos a medida que el pipeline los necesita; al primer uso se desarrollan ad-hoc en el borrador, al segundo uso se extraen a [`tangibles-snippets.md`](../../content/templates/tangibles-snippets.md) como plantilla canónica reutilizable.

  **Verificación pre-output** (implementada en `/content-draft § 8.7`): el borrador con tangible inline debe contener tanto el preview (con clase CSS del tangible en uso) como el `.snippet-block` correspondiente con HTML inline escapado. Si falta el snippet-block → bloqueo del output.

  **Incidente origen:** borrador #10 *mejor-robot-aspirador-mascotas-2026* v1 entregó el árbol de decisión solo como preview con clases CSS globales. Al pegarlo Rafael a Beehiiv perdió todos los estilos (*"no me queda bien ponerlo como texto directamente desde el newsletter"*). La solución canonizada es: preview + snippet inline para todo tangible, siempre. Patrón ahora es norma — no solo para banners y CTAs.
