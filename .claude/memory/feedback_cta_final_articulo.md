...
description: Todo artículo no-ficción cierra con snippet dark "¿Te ha servido este análisis?" → botón Suscribirse → https://robohogar.com; posición: tras ¿Sabías que? + separator, antes del disclaimer (sin bloque manual "Más en ROBOHOGAR" — Beehiiv inyecta Keep Reading solo); sin UTM
type: feedback
...
**Posición canónica** en el esqueleto del borrador: después del bloque `¿Sabías que…?` + su `<div class="separator"></div>`, antes del disclaimer final. El CTA principal cierra el contenido editorial. **NO incluir bloque manual "Más en ROBOHOGAR"** (regla dura 2026-04-26): Beehiiv inyecta automáticamente bajo cada post un bloque "Keep Reading" con los 3 posts más recientes — duplicarlo manualmente ensucia el cierre.
...
- Verificación pre-output: `grep -c "¿Te ha servido este análisis?" <borrador.html>` → debe ser 1 exacto. `grep -nE '¿Te ha servido[^<]*</div>.*utm_' <borrador.html>` → 0 (sin UTM). `grep -niE 'Más en ROBOHOGAR|Keep Reading' <borrador.html>` → 0 (Beehiiv inyecta Keep Reading solo). Orden en el HTML: Sabías → ¿Te ha servido → disclaimer.
