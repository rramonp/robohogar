...
- `/audiobook-generate <slug>` — genera audiolibro MP3 de un relato de Ficciones con voz Luis ElevenLabs + upload a Cloudflare R2 + chunks-index.json (timestamps capítulos) + 4 strings copy-paste para Beehiiv. **Invocación SIEMPRE MANUAL** sobre texto final aprobado, NUNCA encadenado desde `/ficcion-draft` ni `/post-publish` (economía de API). Detalle: `.claude/commands/audiobook-generate.md`. Plan: `docs/plan-audiolibros-ficciones.md`.
- `/audiobook-distribute <slug>` — distribuye un audiolibro ya generado a YouTube (vídeo D++ híbrido: cover + waveform ámbar + chyron de capítulo cambiante + logo) + RSS feed propio en R2 → Spotify/Apple Podcasts/Amazon Music. Devuelve URLs YouTube + RSS + texto de pinned comment para copy-paste manual (única acción manual recurrente, ~1 min). **Auto-fire desde `/post-publish § paso 13.5`** si el slug es ficción + tiene MP3 + chunks-index.json + covers. También invocable manual. Setup inicial ~2 h en `Docs/Guia Distribucion Audiolibros.md` del vault Obsidian (Google Cloud OAuth + Cloudflare custom domain `feed.robohogar.com` + alta única en plataformas). Detalle: `.claude/commands/audiobook-distribute.md`.
